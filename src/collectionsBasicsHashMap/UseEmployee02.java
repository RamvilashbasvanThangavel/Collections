...
public class UseEmployee02 {
	public static void main(String[] args) {
		...
		int total = 0;
		for (Employee addition : employees.values()) {
			total = total + addition.getsalary();
		}
		System.out.println("Total Salary of all EMployees : " + total);

		Employee max = e1;
		for (Employee hs : employees.values()) {
			if (max.getsalary() < hs.getsalary()) {
				max = hs;
			}
		}
		System.out.println("Highest Salary got by : " + max);

		Employee min = max;
		for (Employee ls : employees.values()) {
			if (min.getsalary() > ls.getsalary()) {
				min = ls;
			}
		}
		System.out.println("Highest Salary got by : " + min);

		for (Employee female : employees.values()) {
			if (female.getgender().equalsIgnoreCase("Female")) {
				System.out.println(female.getname() + "," + female.getsalary());
			}
		}
		for (Employee male : employees.values()) {
			if (male.getgender().equalsIgnoreCase("Male")) {
